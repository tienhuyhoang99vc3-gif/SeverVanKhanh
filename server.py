...
function renderSongList(songs){
  ...
  songs.forEach(function(s,i){
    var cover=s.cover?('<img class="sc-cover" src="'+s.cover+'" alt="" onerror="this.style.display=\'none\'">'):('<div class="sc-cover-placeholder"><i class="fa-solid fa-music"></i></div>');
...
function renderDbTable(){
  ...
  rows.forEach(function(k){
    ...
    html+='<tr>'
      +'<td><span class="key-val" onclick="copyText(\''+k.key+'\')" style="cursor:pointer;" title="Click để copy">'+k.key+'</span></td>'
      ...
      +'<button class="btn-sm btn-sm-warn" onclick="extendKeyPrompt(\''+k.key+'\')"><i class="fa-solid fa-clock"></i></button>'
      +'<button class="btn-sm btn-sm-red" onclick="deleteKey(\''+k.key+'\')"><i class="fa-solid fa-trash"></i></button>'
      +'</div></td>'
      +'</tr>';
  });
    ...
    if(d.status==='success'){
      ...
      keys.forEach(function(k){
        html+='<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);">'
          ...
          +'<button class="btn-sm btn-sm-blue" onclick="copyText(\''+k+'\')"><i class="fa-solid fa-copy"></i> Sao chép</button>'
          +'</div>';
      });
...
function loadFreeKeys(){
  ...
  .then(function(d){
    ...
    var html=keys.map(function(k){
      return '<tr><td><span class="key-val" onclick="copyText(\''+k.key+'\')" style="cursor:pointer;">'+k.key+'</span></td>'
        ...
        +'<td><button class="btn-sm btn-sm-red" onclick="deleteFreeKey(\''+k.key+'\')"><i class="fa-solid fa-trash"></i></button></td>'
        +'</tr>';
    }).join('');
...
function renderDevPending(list){
  ...
  $('devReqList').innerHTML=list.map(function(r){
    return '<div class="dev-req-card">'
      ...
      +'<button class="btn-sm btn-sm-green" onclick="approveDevice(\''+r.key+'\',\''+r.device_id+'\')"><i class="fa-solid fa-check"></i> Duyệt</button>'
      +'<button class="btn-sm btn-sm-red" onclick="rejectDevice(\''+r.key+'\',\''+r.device_id+'\')"><i class="fa-solid fa-ban"></i> Từ chối</button>'
      +'</div></div>';
  }).join('');
}
...
function renderDevApproved(list){
  ...
  $('devApvList').innerHTML=list.map(function(r){
    return '<div class="apv-card">'
      ...
      +'<button class="btn-sm btn-sm-red" onclick="revokeDevice(\''+r.key+'\',\''+r.device_id+'\')"><i class="fa-solid fa-trash"></i> Thu hồi</button>'
      +'</div></div>';
  }).join('');
}
